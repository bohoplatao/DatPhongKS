window.suaController = function($scope, $routeParams, $http, $location, $filter) {
    $scope.title = "Cập nhật thông tin"

    const apiCapNhat = 'http://localhost:3000/DatPhong'

    let datPhongID = $routeParams.id

    $http.get(apiCapNhat + '/' + datPhongID).then(function(response) {
    ...
    $scope.capNhatDatPhong = function() {
        const apiCapNhat = 'http://localhost:3000/DatPhong'
        ...
        if(flag) {
            ...
            $http.put(apiCapNhat + '/' + datPhongID, capNhatDatPhong).then(function(response) {
                if(response.status == 200) {
